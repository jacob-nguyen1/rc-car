...
void PWM_Init(TIM_TypeDef *timer, uint8_t channel, PWM_InitTypeDef *init) {
	...
	switch (channel) {
		case 1:
			...
			timer->CCMR1 |= (init->Mode << TIM_CCMR1_OC1M_Pos);
			timer->CCER |= TIM_CCER_CC1E;
			timer->CCR1 = 0;
			break;
		case 2:
			timer->CCMR1 &= ~(TIM_CCMR1_OC2M);
			timer->CCMR1 |= (init->Mode << TIM_CCMR1_OC2M_Pos);
			timer->CCER |= TIM_CCER_CC2E;
			timer->CCR2 = 0;
			break;
		case 3:
			timer->CCMR2 &= ~(TIM_CCMR2_OC3M);
			timer->CCMR2 |= (init->Mode << TIM_CCMR2_OC3M_Pos);
			timer->CCER |= TIM_CCER_CC3E;
			timer->CCR3 = 0;
			break;
		case 4:
			timer->CCMR2 &= ~(TIM_CCMR2_OC4M);
			timer->CCMR2 |= (init->Mode << TIM_CCMR2_OC4M_Pos);
